...
@php
    $cartBase = CartManager::cart_grand_total(type: 'checked')
        - (session('coupon_discount') ?? 0)
        - CartManager::order_wise_shipping_discount()
        - CartManager::getShippingCostSavedForFreeDelivery(type: 'checked');
    $pesapalFee = CartManager::get_pesapal_transaction_fee($cartBase);
@endphp

<tr id="pesapal-fee-row">
    <td class="text-muted">
        {{ translate('transaction_fee') }}
        <small class="text-danger">(Pesapal 4%)</small>
    </td>
    <td class="text-end fw-semibold text-danger">
        + {{ webCurrencyConverter($pesapalFee) }}
    </td>
</tr>
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                ...
                <h2 class="text-primary">{{webCurrencyConverter($product_price_total+$total_tax+$total_shipping_cost+$pesapalFee-$coupon_dis-$total_discount_on_product-$order_wise_shipping_discount)}}</h2>
            </div>
