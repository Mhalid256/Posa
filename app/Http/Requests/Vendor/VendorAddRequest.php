...
class VendorAddRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            'f_name' => 'required',
            'l_name' => 'required',
            'phone' => 'nullable|unique:sellers|max:20',
    ...
    public function after(): array
    {
        return [
            function (Validator $validator) {
                ...
                if ($numericLength < 4) {
                    $validator->errors()->add(
                        'phone', translate('The_phone_number_must_be_at_least_4_characters')
                    );
                }

                if ($numericLength > 20) {
                    $validator->errors()->add(
                        'phone', translate('The_phone_number_may_not_be_greater_than_20_characters')
                    );
                }
            }
        ];
