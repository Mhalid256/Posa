...
use App\Enums\ViewPaths\Vendor\VendorEmployeeRole as VendorEmployeeRolePaths;
...
Route::group(['middleware' => ['maintenance_mode']], function () {
    ...
    /* Vendor Employee Authentication Routes */
    /* ── Vendor Employee Authentication ── */
            Route::group(['prefix' => 'vendor/employee', 'as' => 'vendor.employee.auth.'], function () {
                Route::controller(VendorEmployeeLoginController::class)->group(function () {
...
//    after the delivery-man group (around line 310 in your routes.php).
//    It follows the exact same pattern as all other vendor route groups.
// =============================================================================
...
Route::group(['prefix' => 'employee', 'as' => 'employee.'], function () {
    ...
    Route::group(['prefix' => 'role', 'as' => 'role.'], function () {
        Route::controller(VendorRoleController::class)->group(function () {
            Route::get(VendorEmployeeRolePaths::INDEX[URI], 'index')->name('index');
            Route::post(VendorEmployeeRolePaths::INDEX[URI], 'add')->name('add');
            Route::get(VendorEmployeeRolePaths::UPDATE[URI] . '/{id}', 'getUpdateView')->name('update');
            Route::post(VendorEmployeeRolePaths::UPDATE[URI] . '/{id}', 'update');
            Route::post('status-update', 'updateStatus')->name('status-update');
    ...
    Route::controller(VendorEmployeeController::class)->group(function () {
        Route::get(VendorEmployeePaths::LIST[URI], 'index')->name('list');
        Route::get(VendorEmployeePaths::ADD[URI], 'getAddView')->name('add');
        Route::post(VendorEmployeePaths::ADD[URI], 'add');
        Route::get(VendorEmployeePaths::VIEW_DETAILS[URI] . '/{id}', 'getView')->name('view');
        Route::get(VendorEmployeePaths::UPDATE[URI] . '/{id}', 'getUpdateView')->name('update');
        Route::post(VendorEmployeePaths::UPDATE[URI] . '/{id}', 'update');
        Route::post('status-update', 'updateStatus')->name('status-update');
        Route::delete('delete', 'delete')->name('delete');
    });
});
