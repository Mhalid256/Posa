...
// In routes/web.php
Route::post('/cart/set-payment-method', function(\Illuminate\Http\Request $req) {
   session(['selected_payment_method' => $req->input('method')]);
...
if (!$isGatewayPublished) {
    Route::group(['prefix' => 'payment'], function () {
        ...
        Route::group(['prefix' => 'pesapal', 'as' => 'pesapal.'], function () {
            Route::get('pay', [\App\Http\Controllers\Payment_Methods\PesapalController::class, 'pay'])->name('pay');
            Route::get('callback', [\App\Http\Controllers\Payment_Methods\PesapalController::class, 'callback'])->name('callback');
            Route::get('cancel', [\App\Http\Controllers\Payment_Methods\PesapalController::class, 'cancel'])->name('cancel');
            Route::post('ipn', [\App\Http\Controllers\Payment_Methods\PesapalController::class, 'ipn'])
                ->name('ipn')
        ->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class]);
});
    });
